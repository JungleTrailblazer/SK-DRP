...
class GFC(nn.Module):
    def __init__(self, args, ent2id, rel2id, triples):
        ...
        try:
            if args.bert_name == "bert-base-uncased":
                self.bert_encoder = BertModel.from_pretrained(os.environ.get('BERT_BASE_UNCASED', 'bert-base-uncased'))
            elif args.bert_name == "roberta-base":
                self.bert_encoder = RobertaModel.from_pretrained(os.environ.get('ROBERTA_BASE', 'roberta-base'))
            else:
    ...
    def forward(self, heads, questions, answers=None, entity_range=None, hops=None, ls=None):
