...
def train(args):
    logging.info("curriculum learning")
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    print(device)
    path_abs = os.environ.get('SK_DRP_ROOT', os.path.abspath(os.path.join(os.getcwd(), '../')))
    ...
    for epoch in range(args.num_epoch):
        ...
        for iteration, batch in enumerate(train_loader):
            #print(batch[0].shape, train_loader.batch_size)
            #print(backward, iteration, (iteration+1) * train_loader.batch_size, len(train_loader)*train_loader.batch_size-epoch*155, (len(train_loader)*train_loader.batch_size-(20-t)*155) )
            '''if not backward and (iteration+1) * train_loader.batch_size > len(train_loader)*train_loader.batch_size-epoch*155:
                if (iteration+1) * train_loader.batch_size > 2787:
                    backward = True
                continue
            elif backward and (iteration+1) * train_loader.batch_size < (len(train_loader)*train_loader.batch_size-(20-t)*155):
                continue'''
            if not finish:
                if not backward and (iteration + 1) * train_loader.batch_size > (epoch + 1) * 155:
                    continue
                elif backward and (iteration + 1) * train_loader.batch_size < t * 155:
                    if (iteration + 1) == len(train_loader) - 1:
                        finish = True
                    continue
                if not backward and (iteration + 1) == len(train_loader):
                    backward = True

            #print(backward, iteration, (iteration+1) * train_loader.batch_size, (epoch+1)*155, t*155)
